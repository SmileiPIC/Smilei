...
\item \code{data\_log} = \code{True} or (\code{False})       (optional)\\*
	If True, then $\log10$ is applied to the output array.
\end{itemize}
...
\subsection{Other tips on managing plots}
After you have prepared a diagnostic with \code{ParticleDiagnostic(...)}, you \textbf{cannot} change anymore the following arguments:
\code{results\_path}, \code{diagNumber}, \code{timesteps}, \code{slice}, \code{units} and \code{data\_log}.\\*
However, you can change the plotting parameters using the \code{.plot()} or the \code{.set()} functions:
\begin{lstlisting}
...
Another cool thing is that you can change the colormap for 2D plots, with the \code{cmap} argument.
For example, you can plot with a gray colormap using
\begin{lstlisting}
>>> A.plot( cmap = "gray" )
\end{lstlisting}
Many colormaps are available from the \textit{matplotlib} package. With \code{cmap=""}, you will get an error \code{Colormap  is not recognized}, but it lists all the available colormaps.
