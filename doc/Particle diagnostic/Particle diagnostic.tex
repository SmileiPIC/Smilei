...
\clearpage
\section{Bonus: view fields as well}
...
All the functions \code{plot()}, \code{set()}, \code{get()} and \code{getData()} are also compatible with \code{Field(...)}.\\*\\*

Last but not least: \code{multiPlot()} can be used to combine field and particle diagnostics on the same figure.\\*\\*
