...
\textbf{\underline{To prepare the data:}}
\begin{lstlisting}
...
\textbf{\underline{To obtain the data as an array:}}
\begin{lstlisting}
...
\textbf{\underline{To plot the data:}}
\begin{lstlisting}
...
\textbf{\underline{To simultaneously plot multiple diagnostics in the same figure:}}
\begin{lstlisting}
...
An example of the commands you may use from a \textit{terminal} (or \textit{console}, or \textit{command line window}) is
\begin{lstlisting}
...
We want to have a look at the diagnostic \#0, which is the density of species \#1 (here, electrons) with two
...
\end{lstlisting}
A window appears showing the plot in Figure \ref{Tuto1}. We can see that the electrons have indeed a drift velocity of $0.05c$.
