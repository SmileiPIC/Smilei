...
where :math:`E_s = m^2 c^3 / \hbar e \simeq 1.3 \cdot 10^{18}\ \mathrm{V/m}` is
the Schwinger field, :math:`e` the electron charge,
:math:`m` the electron mass, :math:`c` the speed of light in vacuum,
:math:`\hbar` the reduced Planck constant, :math:`\mathbf{E} = (E_x, E_y, E_z)`
and :math:`\mathbf{B} = (B_x, B_y, B_z)` the electric and magnetic fields,
:math:`\gamma = \varepsilon_\pm / m c^2` the particle
Lorentz factor (and the normalized particle energy); and :math:`\beta = v/c`
...
Condition (ii) corresponds to :math:`\mathbf{B}^2 - \mathbf{E}^2 \ll E_s^2`
