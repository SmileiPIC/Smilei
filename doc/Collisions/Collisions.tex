...
\newcommand{\vbar}[1]{\hspace*{#1 cm}\vspace{-4pt}$|$\\*}
\newcommand{\corner}[1]{\hspace*{#1 cm}\hspace{0.7pt}\raisebox{3pt}{$\llcorner$}\raisebox{0.2pt}{$\!-$}}
\newlength{\tmp}
...
\vbar{0}
\corner{0} For each particle cluster:\\*
\vbar{1}
\corner{1} If \textit{intra-collisions}\\*
...
\vbar{1}
\corner{1} The two resulting arrays represent pairs of particles (see algorithm in Ref. [3]).\\*
\vbar{1}
\corner{1} Calculate a few intermediate quantities:\\*
\spacing{2} Particle density $n_1$ of group 1.\\*
\spacing{2} Particle density $n_1$  of group 2.\\*
\spacing{2} ``crossed'' particle density $n_{12}$ (see Ref. [1]).\\*
\spacing{2} Other constants.\\*
\vbar{1}
\corner{1} For each pair of particles:\\*
\vbar{2}
\corner{2} Calculate the momenta in the center-of-mass (COM) frame.\\*
\vbar{2}
\corner{2} Calculate the coulomb log if requested (see Ref. [1]).\\*
\vbar{2}
\corner{2} Calculate the collision parameter $s$ and its correction at low temperature (see Ref. [1]).\\*
\vbar{2}
\corner{2} Pick the deflection angle (see Ref. [2]).\\*
\vbar{2}
\corner{2} Deflect particles in the COM frame and go back to the laboratory frame.\\*
...
The black lines correspond to the theoretical rates taken from the NRL formulary:\\*
\begin{equation*}
\nu_T=\frac{e^4 \,n_e\,\ln\Lambda } { 8\pi^{3/2} \epsilon_0^2 \,m_e^{1/2}T_e^{3/2} } A^{-2} \left[-3+(3-A)\frac{\rm{arctanh}(\sqrt{A})}{\sqrt{A}}\right]
...
An electric field of $E=$ 3.2 V/m (0.001 in code units) is applied.\\*
