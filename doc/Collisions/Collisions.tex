...
\vfill
The \blue{optional} value \val{ln$\Lambda$} must be a float.
\begin{itemize}
...
\vfill
The \blue{optional} value \val{debug\_every} must be an integer: the number of timesteps between each output of
information about collisions. If absent or zero, there will be no outputs. See section \ref{sec:debug}
\clearpage
