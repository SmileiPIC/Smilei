...
class RadiationSpectrum(Diagnostic):
	...
	def _init(self, diagNumber=None, timesteps=None, subset=None, sum=None, data_log=False, include={}, **kwargs):
		...
		for d in self._diags:
			...
			for path in self._results_path:
				f = self._h5py.File(path+self._os.sep+'RadiationSpectrum'+str(d)+'.h5')
				items.update( dict(f) )
	...
	def _getInfo(self,diagNumber):
		...
		for path in self._results_path:
			# Open file
			try:
				file = path+self._os.sep+'RadiationSpectrum'+str(diagNumber)+'.h5'
			...
			time_average = int(f.attrs["time_average"])
			species = bytes.decode(f.attrs["species"].strip()).split() # get all species numbers
			species = [int(s) for s in species]
			n = 0
			axes = []
			while "axis"+str(n) in f.attrs:
				sp = bytes.decode(f.attrs["axis"+str(n)]).split()
				axes.append({
					"type":sp[0],
					"min":float(sp[1]),
					"max":float(sp[2]),
					"size":int(sp[3]),
					"log":bool(int(sp[4])),
					"edges_included":bool(int(sp[5]))
				})
				n += 1
			sp = bytes.decode(f.attrs["photon_energy_axis"]).split()
			axes.append({
				"type":"gamma",
				"min":float(sp[0]),
				"max":float(sp[1]),
				"size":int(sp[2]),
				"log":bool(int(sp[3])),
				"edges_included":bool(int(sp[4]))
			})
			f.close()
			# Verify that the info corresponds to the diag in the other paths
			if info == {}:
				info = {"#":diagNumber, "tavg":time_average, "species":species, "axes":axes}
			else:
				if axes!=info["axes"]:
	...
	def getDiags(self):
		allDiags = []
		for path in self._results_path:
			files = self._glob(path+self._os.sep+"RadiationSpectrum*.h5")
			diags = [int(self._re.findall(r"RadiationSpectrum([0-9]+)[.]h5$",file)[0]) for file in files]
			if len(allDiags)>0:
