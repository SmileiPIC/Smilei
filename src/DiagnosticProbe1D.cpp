...
void DiagnosticProbe1D::close() {
    if (fileId>0) {
        for(unsigned int i=0;i<dataId.size();i++){
            H5Dclose(dataId[i]);
        }
        H5Fclose(fileId);
    }
...
probeSize(7),
fileId(0),
dataId(diagParams->probe1DStruc.size())
{
    ...
    for (unsigned int np=0; np<diagParams->probe1DStruc.size(); np++) {
        ...
        unsigned int nProbes=diagParams->probe1DStruc[np].number;

        probeId[np].resize(nProbes);
        
        hsize_t dims[3] = {0, probeSize, nProbes};
        hsize_t max_dims[3] = {H5S_UNLIMITED, probeSize, nProbes};
        hid_t file_space = H5Screate_simple(3, dims, max_dims);

        hid_t plist = H5Pcreate(H5P_DATASET_CREATE);
        H5Pset_layout(plist, H5D_CHUNKED);
        hsize_t chunk_dims[3] = {1, probeSize, nProbes};
        H5Pset_chunk(plist, 3, chunk_dims);

        
        unsigned int ndim=params->nDim_particle;

        hsize_t dimsPos[2] = {ndim, nProbes};
        hid_t dataspace_id = H5Screate_simple(2, dimsPos, NULL);
        
        probeParticles[np].initialize(nProbes, ndim);
        
        double *tmp= new double[ndim*nProbes];
        for(unsigned int count=0; count!=nProbes; ++count) {
            int found=smpi_->getRank();
            for(unsigned int iDim=0; iDim!=ndim; ++iDim) {
                if (nProbes>1) {
                    tmp[iDim+ndim*count]=diagParams->probe1DStruc[np].posStart[iDim]+count*(diagParams->probe1DStruc[np].posEnd[iDim]-diagParams->probe1DStruc[np].posStart[iDim])/(nProbes-1);
                }else {
                    tmp[iDim+ndim*count]=0.5*(diagParams->probe1DStruc[np].posStart[iDim]+diagParams->probe1DStruc[np].posEnd[iDim]);
                }
                
                if(smpi_->getDomainLocalMin(iDim) >  tmp[iDim+ndim*count] || smpi_->getDomainLocalMax(iDim) <= tmp[iDim+ndim*count]) {
                    found=-1;
                }
            }
            ...
            for(unsigned int iDim=0; iDim!=ndim; ++iDim) {
                probeParticles[np].position(iDim,count)=tmp[iDim+ndim*count];
                tmp[iDim+ndim*count]/=2*M_PI;
            }
        }
