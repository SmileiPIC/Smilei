...
// ---------------------------------------------------------------------------------------------------------------------
// Creator for Interpolator3DWT2OrderV
// ---------------------------------------------------------------------------------------------------------------------
Interpolator3DWT2OrderV::Interpolator3DWT2OrderV( Params &params, Patch *patch ) : Interpolator3DWT2Order( params, patch )
{
...
// ---------------------------------------------------------------------------------------------------------------------
// 2nd OrderV WT Interpolation of the fields at a the particle position (3 nodes are used)
// ---------------------------------------------------------------------------------------------------------------------
// void Interpolator3DWT2OrderV::fields( ElectroMagn *EMfields, Particles &particles, int ipart, double *ELoc, double *BLoc )
// {
// }

void Interpolator3DWT2OrderV::fieldsWrapper( ElectroMagn * __restrict__ EMfields,
                                             Particles &particles,
                                             ...
                                             int ipart_ref )
{
    ...
    for( int iivect=0 ; iivect<nbVec; iivect++ ) {
        ...
        #pragma omp simd private(var1, delta)
        for( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            var1 = D_ov_8dt[0] * ( fabs( dt_ov_D[0]-delta ) * ( dt_ov_D[0]-delta ) + fabs( dt_ov_D[0]+delta ) * ( dt_ov_D[0]+delta ) );
            coeff[0][0][0][ipart]    = var1 - 0.5 * delta;
            coeff[0][0][1][ipart]    = 1.0 - 2.0 * var1;
            coeff[0][0][2][ipart]    = var1 + 0.5 * delta;
            //store delta primal in global array
            //deltaO[i][ipart-ipart_ref+ivect+istart[0]] = delta;
            ...
            coeff[0][1][0][ipart]    =  0.5 * ( var1-delta+0.25 );
            coeff[0][1][1][ipart]    = ( 0.75 - var1 );
            coeff[0][1][2][ipart]    =  0.5 * ( var1+delta+0.25 );

            // Y direction

            //delta primal = distance to primal node
            delta   = position_y[ipart2]*d_inv_[1] - idx[1];
            var1 = D_ov_8dt[1] * ( fabs( dt_ov_D[1]-delta ) * ( dt_ov_D[1]-delta ) + fabs( dt_ov_D[1]+delta ) * ( dt_ov_D[1]+delta ) );
            coeff[1][0][0][ipart]    = var1 - 0.5 * delta;
            coeff[1][0][1][ipart]    = 1.0 - 2.0 * var1;
            coeff[1][0][2][ipart]    = var1 + 0.5 * delta;
            //store delta primal in global array
            ...
            coeff[1][1][0][ipart]    =  0.5 * ( var1-delta+0.25 );
            coeff[1][1][1][ipart]    = ( 0.75 - var1 );
            coeff[1][1][2][ipart]    =  0.5 * ( var1+delta+0.25 );

            // Z direction

            //delta primal = distance to primal node
            delta   = position_z[ipart2]*d_inv_[2] - idx[2];
            var1 = D_ov_8dt[2] * ( fabs( dt_ov_D[2]-delta ) * ( dt_ov_D[2]-delta ) + fabs( dt_ov_D[2]+delta ) * ( dt_ov_D[2]+delta ) );
            coeff[2][0][0][ipart]    = var1 - 0.5 * delta;
            coeff[2][0][1][ipart]    = 1.0 - 2.0 * var1;
            coeff[2][0][2][ipart]    = var1 + 0.5 * delta;
            //store delta primal in global array
            ...
            coeff[2][1][0][ipart]    =  0.5 * ( var1-delta+0.25 );
            coeff[2][1][1][ipart]    = ( 0.75 - var1 );
            coeff[2][1][2][ipart]    =  0.5 * ( var1+delta+0.25 );
        ...
        double * __restrict__ coeffxpt = &( coeff[0][0][1][0] );
        double * __restrict__ coeffypt = &( coeff[1][0][1][0] );
        double * __restrict__ coeffyd  = &( coeff[1][1][1][0] );
        double * __restrict__ coeffzpt = &( coeff[2][0][1][0] );
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    ...
                    for( int kloc=0 ; kloc<3 ; kloc++ ) {
                         interp_res += coeffxd[ipart+(iloc-1)*32] * coeffypt[ipart+(jloc-1)*32]  * coeffzpt[ipart + (kloc-1)*32] *
        ...
        for ( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    UNROLL_S(3)
                    for( int kloc=0 ; kloc<3 ; kloc++ ) {
                        interp_res += coeffxpt[ipart+(iloc-1)*32] * coeffyd[ipart+(jloc-1)*32] * coeffzpt[ipart + (kloc-1)*32] *
        ...
        for ( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    UNROLL_S(3)
                    for( int kloc=0 ; kloc<3 ; kloc++ ) {
                        interp_res += coeffxpt[ipart+(iloc-1)*32] * coeffypt[ipart+(jloc-1)*32] * coeffzd[ipart + (kloc-1)*32] *
        ...
        for ( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    UNROLL_S(3)
                    for( int kloc=0 ; kloc<3 ; kloc++ ) {
                        interp_res += coeffxpt[ipart+(iloc-1)*32] * coeffyd[ipart+(jloc-1)*32] * coeffzd[ipart + (kloc-1)*32] *
        ...
        for ( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    UNROLL_S(3)
                    for( int kloc=0 ; kloc<3 ; kloc++ ) {
                        interp_res += coeffxd[ipart+(iloc-1)*32] * coeffypt[ipart+(jloc-1)*32] * coeffzd[ipart + (kloc-1)*32] *
        ...
        for ( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int iloc=0 ; iloc<3 ; iloc++ ) {
                ...
                for( int jloc=0 ; jloc<3 ; jloc++ ) {
                    UNROLL_S(3)
                    for( int kloc=0; kloc<3 ; kloc++ ) {
                        interp_res += coeffxd[ipart+(iloc-1)*32] * coeffyd[ipart+(jloc-1)*32] * coeffzpt[ipart + (kloc-1)*32] *
...
} // END Interpolator3DWT2OrderV


void Interpolator3DWT2OrderV::fieldsAndCurrents( ElectroMagn * __restrict__ EMfields,
                                                 Particles &particles,
                                                 SmileiMPI * __restrict__ smpi,
                                                 int * __restrict__ istart,
                                                 int * __restrict__ iend,
                                                 ...
                                                 double * __restrict__ RhoLoc )
{
    ...
    for( int i=0; i<3; i++ ) { // for X/Y
        ...
        var1  = delta*delta;
        coeff[i][0][0]    =  0.5 * ( var1-delta+0.25 );
        coeff[i][0][1]    = ( 0.75 - var1 );
        coeff[i][0][2]    =  0.5 * ( var1+delta+0.25 );
        dual [i] = ( delta >= 0. );

        // WT
        var1 = D_ov_8dt[i] * ( fabs( dt_ov_D[i]-delta ) * ( dt_ov_D[i]-delta ) + fabs( dt_ov_D[i]+delta ) * ( dt_ov_D[i]+delta ) );
        coeff[i][1][0] = var1 - 0.5 * delta;
        coeff[i][1][1] = 1.0 - 2.0 * var1;
        coeff[i][1][2] = var1 + 0.5 * delta;
        
        //delta dual = distance to dual node
        delta   = delta - dual[i] + 0.5 ;
        var1  = delta*delta;

        coeff[i][2][0]    =  0.5 * ( var1-delta+0.25 );
        coeff[i][2][1]    = ( 0.75 - var1 );
        coeff[i][2][2]    =  0.5 * ( var1+delta+0.25 );
    }

    double * __restrict__ coeffyp  = &( coeff[1][0][1] );
    double * __restrict__ coeffypt = &( coeff[1][1][1] );
    double * __restrict__ coeffyd  = &( coeff[1][2][1] );
    double * __restrict__ coeffxd  = &( coeff[0][2][1] );
    double * __restrict__ coeffxpt = &( coeff[0][1][1] );
    double * __restrict__ coeffxp  = &( coeff[0][0][1] );
    double * __restrict__ coeffzp  = &( coeff[2][0][1] );
    double * __restrict__ coeffzpt = &( coeff[2][1][1] );
    double * __restrict__ coeffzd  = &( coeff[2][2][1] );
...
void Interpolator3DWT2OrderV::fieldsAndEnvelope( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
    if( istart[0] == iend[0] ) {
        return;    //Don't treat empty cells.
    }
    ...
    for( int iivect=0 ; iivect<nbVec; iivect++ ) {
        ...
        for( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            for( int i=0; i<3; i++ ) { // for X/Y
                ...
                var1  = delta*delta;
                coeff[i][0][0][ipart]    =  0.5 * ( var1-delta+0.25 );
                coeff[i][0][1][ipart]    = ( 0.75 - var1 );
                coeff[i][0][2][ipart]    =  0.5 * ( var1+delta+0.25 );
                deltaO[i][ipart-ipart_ref+ivect+istart[0]] = delta;

                // for primal (WT)
                var1 = D_ov_8dt[i] * ( fabs( dt_ov_D[i]-delta ) * ( dt_ov_D[i]-delta ) + fabs( dt_ov_D[i]+delta ) * ( dt_ov_D[i]+delta ) );
                coeff[i][1][0][ipart]    = var1 - 0.5 * delta;
                coeff[i][1][1][ipart]    = 1.0 - 2.0 * var1;
                coeff[i][1][2][ipart]    = var1 + 0.5 * delta;

                // for dual
                delta   = delta0 - ( double )idx[i] + ( 0.5-dual[i][ipart] );
                var1  = delta*delta;
                coeff[i][2][0][ipart]    =  0.5 * ( var1-delta+0.25 );
                coeff[i][2][1][ipart]    = ( 0.75 - var1 );
                coeff[i][2][2][ipart]    =  0.5 * ( var1+delta+0.25 );
        ...
        for( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            double *coeffypt = &( coeff[1][1][1][ipart] );
            double *coeffyd  = &( coeff[1][2][1][ipart] );
            double *coeffxd  = &( coeff[0][2][1][ipart] );
            double *coeffxpt = &( coeff[0][1][1][ipart] );
            double *coeffxp  = &( coeff[0][0][1][ipart] );
            double *coeffzp  = &( coeff[2][0][1][ipart] );
            double *coeffzpt = &( coeff[2][1][1][ipart] );
            double *coeffzd  = &( coeff[2][2][1][ipart] );
            ...
            for( int iloc=-1 ; iloc<2 ; iloc++ ) {
                for( int jloc=-1 ; jloc<2 ; jloc++ ) {
                    for( int kloc=-1 ; kloc<2 ; kloc++ ) {
                        //interp_res += *(coeffxpt+iloc*32) * *(coeffyd+jloc*32) * *(coeffzpt+kloc*32) *
                        interp_res += *( coeffxpt+iloc*32 ) * *( coeffyd+jloc*32 ) * *( coeffzd+kloc*32 ) *
            ...
            for( int iloc=-1 ; iloc<2 ; iloc++ ) {
                for( int jloc=-1 ; jloc<2 ; jloc++ ) {
                    for( int kloc=-1 ; kloc<2 ; kloc++ ) {
                        //interp_res += *(coeffxpt+iloc*32) * *(coeffyd+jloc*32) * *(coeffzpt+kloc*32) *
                        interp_res += *( coeffxd+iloc*32 ) * *( coeffypt+jloc*32 ) * *( coeffzd+kloc*32 ) *
            ...
            for( int iloc=-1 ; iloc<2 ; iloc++ ) {
                for( int jloc=-1 ; jloc<2 ; jloc++ ) {
                    for( int kloc=-1 ; kloc<2 ; kloc++ ) {
                        interp_res += *( coeffxd+iloc*32 ) * *( coeffyd+jloc*32 ) * *( coeffzpt+kloc*32 ) *
...
void Interpolator3DWT2OrderV::timeCenteredEnvelope( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
    if( istart[0] == iend[0] ) {
        return;    //Don't treat empty cells.
    }
...
void Interpolator3DWT2OrderV::envelopeAndSusceptibility( ElectroMagn *EMfields, Particles &particles, int ipart, double *Env_A_abs_Loc, double *Env_Chi_Loc, double *Env_E_abs_Loc, double *Env_Ex_abs_Loc )
{
    // probes are interpolated one by one for now
...
void Interpolator3DWT2OrderV::oneField( Field **field, Particles &particles, int *istart, int *iend, double *FieldLoc, double *l1, double *l2, double *l3 )
{
