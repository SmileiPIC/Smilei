...
Interpolator1DWT2OrderV::Interpolator1DWT2OrderV( Params &params, Patch *patch ) : Interpolator1D( params, patch )
{
...
// ---------------------------------------------------------------------------------------------------------------------
void Interpolator1DWT2OrderV::fields( ElectroMagn *EMfields, Particles &particles, int ipart, int nparts, double *ELoc, double *BLoc )
{
                                          ...
                                          SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
    ...
    #pragma omp simd private(xjn, var1, coeffd, coeffpt, idx, ipx)
    for( int ipart=*istart ; ipart<*iend; ipart++ ) {
        ...
        // Interpolate the fields from the Primal (WT) grid : Ey, Ez, Bx
        Epart_y[ipart] = coeffpt[0] * Ey[ipx-1]   + coeffpt[1] * Ey[ipx]   + coeffpt[2] * Ey[ipx+1];
        Epart_z[ipart] = coeffpt[0] * Ez[ipx-1]   + coeffpt[1] * Ez[ipx]   + coeffpt[2] * Ez[ipx+1];
        Bpart_x[ipart] = coeffpt[0] * Bx[ipx-1]   + coeffpt[1] * Bx[ipx]   + coeffpt[2] * Bx[ipx+1];
...
void Interpolator1DWT2OrderV::fieldsSelection( ElectroMagn *EMfields, Particles &particles, double *buffer, int offset, vector<unsigned int> *selection )
{
    if( selection ) {
...
void Interpolator1DWT2OrderV::fieldsAndEnvelope( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
...
void Interpolator1DWT2OrderV::timeCenteredEnvelope( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
    // Static cast of the envelope fields
...
void Interpolator1DWT2OrderV::envelopeAndSusceptibility( ElectroMagn *EMfields, Particles &particles, int ipart, double *Env_A_abs_Loc, double *Env_Chi_Loc, double *Env_E_abs_Loc, double *Env_Ex_abs_Loc )
{
    // Static cast of the electromagnetic fields
...
void Interpolator1DWT2OrderV::envelopeFieldForIonization( ElectroMagn *EMfields, Particles &particles, SmileiMPI *smpi, int *istart, int *iend, int ithread, int ipart_ref )
{
    // Static cast of the envelope fields
