...
#define ERROR(__txt) {int __rk; MPI_Comm_rank( MPI_COMM_WORLD, &__rk ); __header("ERROR proc "<<__rk, __txt); MPI_Abort( MPI_COMM_WORLD, -1 ); raise(SIGSEGV);}
...
#define ERROR(__txt) {__header("ERROR", __txt); MPI_Abort( MPI_COMM_WORLD, -1 ); exit(EXIT_FAILURE);}
