...
void DiagnosticProbes::openFile( Params& params, SmileiMPI* smpi, VectorPatch& vecPatches, bool newfile )
{
	...
	vector<unsigned int> vecNumber; 
	PyTools::extract("number",vecNumber,"DiagProbe",probeId_);
	// Dimension of the probe grid
	unsigned int dimProbe=vecNumber.size();
	...
	if (PyTools::extract("pos",pos,"DiagProbe",probeId_)) {
	    allPos.push_back(pos);
	}
            
	if (PyTools::extract("pos_first",pos,"DiagProbe",probeId_)) {
	    allPos.push_back(pos);
	}
            
	if (PyTools::extract("pos_second",pos,"DiagProbe",probeId_)) {
	    allPos.push_back(pos);
	}
            
	if (PyTools::extract("pos_third",pos,"DiagProbe",probeId_)) {
	    allPos.push_back(pos);
	}
	...
	vector<string> fs;
	if(!PyTools::extract("fields",fs,"DiagProbe",probeId_)) {
	    fs.resize(10);
	    fs[0]="Ex"; fs[1]="Ey"; fs[2]="Ez";
	    fs[3]="Bx"; fs[4]="By"; fs[5]="Bz";
	    fs[6]="Jx"; fs[7]="Jy"; fs[8]="Jz"; fs[9]="Rho";
	...
	for( unsigned int i=0; i<fs.size(); i++) {
	    for( unsigned int j=0; j<i; j++) {
		if( fs[i]==fs[j] ) {
		    ERROR("Probe #"<<probeId_<<": field "<<fs[i]<<" appears twice");
		}
	    }
	    if     ( fs[i]=="Ex" ) locations[0] = i;
	    else if( fs[i]=="Ey" ) locations[1] = i;
		...
		ERROR("Probe #"<<probeId_<<": unknown field "<<fs[i]);
	    }
	}
