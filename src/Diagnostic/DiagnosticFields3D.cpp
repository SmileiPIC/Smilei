...
void DiagnosticFields3D::getField( Patch* patch, int field_index )
{
    // Get current field
    Field3D* field;
    if( time_average>1 ) {
...
void DiagnosticFields3D::writeField( hid_t dset_id, int timestep ) {
    ...
    unsigned int read_position, write_position, read_skip, write_skip_y, write_skip_z, sx, sy, sz;
