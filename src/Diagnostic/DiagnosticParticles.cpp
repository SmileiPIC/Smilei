...
void DiagnosticParticles::run(int timestep, vector<Species*>& vecSpecies)
{
    ...
    int rk(0);
    MESSAGE( "Add patch in constructor to init a patch master attribute" );
    ...
    if (timestep % every == time_average-1) {
        ...
        MPI_Reduce( (rk==0)?MPI_IN_PLACE:&data_sum[0], &data_sum[0], output_size, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
