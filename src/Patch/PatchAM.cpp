...
void PatchAM::exchangeField_movewin( Field* field, int clrw )
{
    ...
    if (neighbor_[iDim][iNeighbor]!=MPI_PROC_NULL) {
        ...
        ix = (1-iDim)*istart;
        iy =    iDim *istart;
        MPI_Bsend(  &( ( *f2D )( ix, iy ) ), 1, ntype, neighbor_[iDim][iNeighbor], 0, MPI_COMM_WORLD);
    } // END of Send
    ...
    if (neighbor_[iDim][(iNeighbor+1)%2]!=MPI_PROC_NULL) {
        ...
        ix = (1-iDim)*istart;
        iy =    iDim *istart;
        MPI_Irecv(  &( ( *f2D )( ix, iy ) ), 1, ntype, neighbor_[iDim][(iNeighbor+1)%2], 0, MPI_COMM_WORLD, &rrequest);
    } // END of Recv
