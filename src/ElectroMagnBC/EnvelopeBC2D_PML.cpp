    ...
    if( i_boundary_ == 0 && patch->isXmin() ) {
        ...
        for ( int i=min2exchange ; i<max2exchange ; i++ ) {
            for ( int j=1 ; j<ny_p-1 ; j++ ) {
                (*A2D_n_)(ncells_pml_domain-1-domain_oversize_x-nsolver/2+i,j) = (*A2D_n_domain)(i,j);
            }
        }
        ...
        for (int i=0 ; i < nsolver/2 ; i++){
            for ( int j=0 ; j<ny_p ; j++ ) {
                (*A2D_np1_domain)(i,j) = (*A2D_n_)(ncells_pml_domain-1-domain_oversize_x-nsolver/2+i,j);
                (*A2D_n_domain)(i,j) = (*A2D_nm1_)(ncells_pml_domain-1-domain_oversize_x-nsolver/2+i,j);
        ...
        for ( int i=min2exchange ; i<max2exchange ; i++ ) {
            for ( int j=1 ; j<ny_p-1 ; j++ ) {
                (*A2D_n_)(domain_oversize_x+nsolver/2-i,j) = (*A2D_n_domain)(nx_p-1-i,j);
            }
        }
        ...
        for (int i=0 ; i < nsolver/2 ; i++){
            for ( int j=0 ; j<ny_p ; j++ ) {
                (*A2D_np1_domain)(nx_p-1-i,j) = (*A2D_n_)(domain_oversize_x+nsolver/2-i,j);
                (*A2D_n_domain)(nx_p-1-i,j) = (*A2D_nm1_)(domain_oversize_x+nsolver/2-i,j);
                (*Phi2D_domain)(nx_p-1-i,j) = 0.5*ellipticity_factor*std::abs( (*A2D_np1_domain)(nx_p-1-i,j) )*std::abs( (*A2D_np1_domain)(nx_p-1-i,j) ) ;
            }
        }
    }
        ...
        cField2D* A2D_np1_pml_xmin = NULL;
        cField2D* A2D_n_pml_xmin   = NULL;
        Field2D*  Phi2D_pml_xmin   = NULL;

        cField2D* A2D_np1_pml_xmax = NULL;
        cField2D* A2D_n_pml_xmax   = NULL;
        Field2D*  Phi2D_pml_xmax   = NULL;

        if(ncells_pml_xmin != 0){
            A2D_np1_pml_xmin = pml_fields_xmin->A2D_n_;
            A2D_n_pml_xmin   = pml_fields_xmin->A2D_nm1_;
            Phi2D_pml_xmin   = pml_fields_xmin->Phi2D_;
        }

        if(ncells_pml_xmax != 0){
            A2D_np1_pml_xmax = pml_fields_xmax->A2D_n_;
            A2D_n_pml_xmax   = pml_fields_xmax->A2D_nm1_;
            Phi2D_pml_xmax   = pml_fields_xmax->Phi2D_;
        }
            ...
            if (patch->isXmin()) {
                if(ncells_pml_xmin != 0){
                    for ( int i=0 ; i<ncells_pml_xmin ; i++ ) {
                        int idx_start = 0;
                        // Les qtes Primals
                        (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j) = (*A2D_n_pml_xmin)(i,j);
                    }
                }
            }
            for ( int i=1 ; i<nx_p-1 ; i++ ) {
                int idx_start = ncells_pml_xmin-1*(patch->isXmin());
                (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j) = (*A2D_n_domain)(i,j);
            }
            if (patch->isXmax()) {
                if(ncells_pml_xmax != 0){
                    for ( int i=0 ; i<ncells_pml_xmax ; i++ ) {
                        int idx_start = (ypml_size_in_x-1)-(ncells_pml_xmax-1) ;
                        (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j) = (*A2D_n_pml_xmax)(domain_oversize_x+nsolver/2+i,j);
        ...
        for (int j=0 ; j < nsolver/2 ; j++){
            for ( int i=1 ; i<nx_p-1 ; i++ ) { // From i=0 to i=1028 with ypml_size_in_x=1048 
                int idx_start = ncells_pml_xmin-1*(patch->isXmin());
                (*A2D_np1_domain)(i,j) = (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                (*A2D_n_domain)(i,j) = (*A2D_nm1_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                (*Phi2D_domain)(i,j) = 0.5*ellipticity_factor*std::abs( (*A2D_np1_domain)(i,j) )*std::abs( (*A2D_np1_domain)(i,j) );
            }
        }
            ...
            if (patch->isXmin()) {
                if(ncells_pml_xmin != 0){
                    for ( int i=0 ; i<ncells_pml_domain_xmin ; i++ ) {
                        int idx_start = 0;
                        // Primals
                        (*A2D_np1_pml_xmin)(i,j) = (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                        (*A2D_n_pml_xmin)(i,j) = (*A2D_nm1_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                    }
                }
            }
        }
            ...
            if (patch->isXmax()) {
                if(ncells_pml_xmax != 0){
                    for ( int i=0 ; i<ncells_pml_domain_xmax ; i++ ) {
                        ...
                        // Les qtes Primals
                        (*A2D_np1_pml_xmax  )(i,j) = (*A2D_n_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                        (*A2D_n_pml_xmax  )(i,j) = (*A2D_nm1_)(idx_start+i,ncells_pml_domain-1-domain_oversize_y-nsolver/2+j);
                    }
                }
            }
        }
            ...
            if (patch->isXmin()) {
                if(ncells_pml_xmin != 0){
                    for ( int i=0 ; i<ncells_pml_xmin ; i++ ) {
                        int idx_start = 0;
                        // Les qtes Primals
                        (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j) = (*A2D_n_pml_xmin)(i,ny_p-1-j);
                    }
                }
            }
            for ( int i=1 ; i<nx_p-1 ; i++ ) {
                int idx_start = ncells_pml_xmin-1*(patch->isXmin());
                (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j) = (*A2D_n_domain)(i,ny_p-1-j);
            }
            if (patch->isXmax()) {
                if(ncells_pml_xmax != 0){
                    for ( int i=0 ; i<ncells_pml_xmax ; i++ ) {
                        int idx_start = (ypml_size_in_x-1)-(ncells_pml_xmax-1) ;
                        (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j) = (*A2D_n_pml_xmax)(domain_oversize_x+nsolver/2+i,ny_p-1-j);
        ...
        for (int j=0 ; j < nsolver/2 ; j++){
            for ( int i=1 ; i<nx_p-1 ; i++ ) {
                int idx_start = ncells_pml_xmin-1*(patch->isXmin());
                (*A2D_np1_domain)(i,ny_p-1-j) = (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                (*A2D_n_domain)(i,ny_p-1-j) = (*A2D_nm1_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                (*Phi2D_domain)(i,ny_p-1-j) = 0.5*ellipticity_factor*std::abs( (*A2D_np1_domain)(i,ny_p-1-j) )*std::abs( (*A2D_np1_domain)(i,ny_p-1-j) );
            }
        }
            ...
            if (patch->isXmin()) {
                if(ncells_pml_xmin != 0){
                    for ( int i=0 ; i<ncells_pml_domain_xmin ; i++ ) {
                        int idx_start = 0;
                        // Primals
                        (*A2D_np1_pml_xmin)(i,ny_p-1-j) = (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                        (*A2D_n_pml_xmin)(i,ny_p-1-j) = (*A2D_nm1_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                    }
                }
            }
        }
            ...
            if (patch->isXmax()) {
                if(ncells_pml_xmax != 0){
                    for ( int i=0 ; i<ncells_pml_domain_xmax ; i++ ) {
                        int idx_start = ypml_size_in_x-ncells_pml_domain_xmax ;
                        // Les qtes Primals
                        (*A2D_np1_pml_xmax  )(i,ny_p-1-j) = (*A2D_n_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                        (*A2D_n_pml_xmax  )(i,ny_p-1-j) = (*A2D_nm1_)(idx_start+i,domain_oversize_y+nsolver/2-j);
                    }
                }
            }
        }
