...
PML_Solver3D_EnvelopeReducedDispersion::PML_Solver3D_EnvelopeReducedDispersion( Params &params )
    : Solver3D( params )
{
    delta = (1.-(dt/dx)*(dt/dx))/3. ;
    // // X-PML
    // kappa_x_max = 1.0 ;
...
PML_Solver3D_EnvelopeReducedDispersion::~PML_Solver3D_EnvelopeReducedDispersion()
{
}

void PML_Solver3D_EnvelopeReducedDispersion::operator()( ElectroMagn * )
{
...
void PML_Solver3D_EnvelopeReducedDispersion::setDomainSizeAndCoefficients( int iDim, int min_or_max, std::vector<unsigned int> dimPrim, int ncells_pml_domain, int startpml, int* ncells_pml_min, int* ncells_pml_max, Patch* )
{
...
void PML_Solver3D_EnvelopeReducedDispersion::compute_A_from_G( LaserEnvelope *envelope, int iDim, int min_or_max, std::vector<unsigned int> dimPrim, unsigned int solvermin, unsigned int solvermax )
{
    const unsigned int nx_p = dimPrim[0];
    const unsigned int ny_p = dimPrim[1];
    const unsigned int nz_p = dimPrim[2];
        ...
        for( unsigned int i=solvermin ; i<solvermax; i++ ) { // x loop
            for( unsigned int j=1 ; j < ny_p-1 ; j++ ) { // y loop
                for( unsigned int k=1 ; k < nz_p-1 ; k++ ) {
                    ...
                    // ----
                    // dA/dx = dA/dx + ik0 A
                    std::complex<double> dA_over_dx_fdtd = (1 + delta) * ( ( *A_n_pml )( i+1, j, k )-( *A_n_pml )( i-1, j, k ) )/(2.*dx)
                                                         - (    delta) * ( ( *A_n_pml )( i+2, j, k )-( *A_n_pml )( i-2, j, k ) )/(4.*dx) ;
                    std::complex<double> dA_over_dx = dA_over_dx_fdtd
                                                      + i1*k0*( *A_n_pml )( i, j, k ) ;
                    // d2A/dx^2 = d2A/dx^2 + 2ik0 dA/dx - k0^2 A
                    std::complex<double> d2A_over_dx2_fdtd = (1 + delta) * ( ( *A_n_pml )( i-1, j, k )-2.*( *A_n_pml )( i, j, k )+( *A_n_pml )( i+1, j, k ) )/(dx*dx)
                                                           - (    delta) * ( ( *A_n_pml )( i-2, j, k )-2.*( *A_n_pml )( i, j, k )+( *A_n_pml )( i+2, j, k ) )/(4*dx*dx) ;
                    std::complex<double> d2A_over_dx2 = d2A_over_dx2_fdtd
                                                        + 2.*i1*k0*(1+delta)*( ( *A_n_pml )( i+1, j, k )-( *A_n_pml )( i-1, j, k ) )/(2.*dx)
                                                        - 2.*i1*k0*(  delta)*( ( *A_n_pml )( i+2, j, k )-( *A_n_pml )( i-2, j, k ) )/(4.*dx)
                                                        - k0*k0*( *A_n_pml )( i, j, k ) ;
                    // d2A/dy^2 = d2A/dy^2
        ...
        for( unsigned int i=2 ; i<nx_p-2; i++ ) { // x loop
            for( unsigned int j=solvermin ; j < solvermax ; j++ ) { // y loop
                for( unsigned int k=2 ; k < nz_p-2 ; k++ ) { // z loop
                    ...
                    std::complex<double> d2A_over_dx2 = d2A_over_dx2_fdtd
                                                        + 2.*i1*k0*(1+delta)*( ( *A_n_pml )( i+1, j, k )-( *A_n_pml )( i-1, j, k ) )/(2.*dx)
                                                        - 2.*i1*k0*(  delta)*( ( *A_n_pml )( i+1, j, k )-( *A_n_pml )( i-1, j, k ) )/(4.*dx)
                                                        - k0*k0*( *A_n_pml )( i, j, k ) ;
                    // dA/dy = dA/dy
