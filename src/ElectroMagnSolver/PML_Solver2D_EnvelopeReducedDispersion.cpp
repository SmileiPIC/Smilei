...
PML_Solver2D_EnvelopeReducedDispersion::PML_Solver2D_EnvelopeReducedDispersion( Params &params )
    : Solver2D( params )
{
...
PML_Solver2D_EnvelopeReducedDispersion::~PML_Solver2D_EnvelopeReducedDispersion()
{
}

void PML_Solver2D_EnvelopeReducedDispersion::operator()( ElectroMagn * )
{
...
void PML_Solver2D_EnvelopeReducedDispersion::setDomainSizeAndCoefficients( int iDim, int min_or_max, std::vector<unsigned int> dimPrim, int ncells_pml_domain, int startpml, int* ncells_pml_min, int* ncells_pml_max, Patch* )
{
...
void PML_Solver2D_EnvelopeReducedDispersion::compute_A_from_G( LaserEnvelope *envelope, int iDim, int min_or_max, std::vector<unsigned int> dimPrim, unsigned int solvermin, unsigned int solvermax )
{
    const unsigned int nx_p = dimPrim[0];
    const unsigned int ny_p = dimPrim[1];
        ...
        for( unsigned int k=0 ; k<1 ; k++ ) {
            ...
            for( unsigned int i=solvermin ; i<solvermax; i++ ) { // x loop
                for( unsigned int j=1 ; j < ny_p-1 ; j++ ) { // y loop
                    ...
                    // ----
                    // dA/dx = dA/dx + ik0 A
                    std::complex<double> dA_over_dx_fdtd = (1. + delta) * ( ( *A_n_pml )( i+1, j )-( *A_n_pml )( i-1, j ) )/(2.*dx) 
                                                               - delta  * ( ( *A_n_pml )( i+2, j )-( *A_n_pml )( i-2, j ) )/(4.*dx) ;
                    // std::complex<double> dA_over_dx = dA_over_dx_fdtd
                    //                                   + i1*k0*( *A_n_pml )( i, j ) ;
                    // d2A/dx^2 = d2A/dx^2 + 2ik0 dA/dx - k0^2 A
                    std::complex<double> d2A_over_dx2_fdtd = (1. + delta) * ( ( *A_n_pml )( i-1, j )-2.*( *A_n_pml )( i, j )+( *A_n_pml )( i+1, j ) )/(dx*dx) 
                                                                  - delta * ( ( *A_n_pml )( i-2, j )-2.*( *A_n_pml )( i, j )+( *A_n_pml )( i+2, j ) )/(4*dx*dx) ;
                    std::complex<double> d2A_over_dx2 = d2A_over_dx2_fdtd
                                                        + 2.*i1*k0* (1.+delta) * ( ( *A_n_pml )( i+1, j )-( *A_n_pml )( i-1, j ) )/(2.*dx)
                                                        - 2.*i1*k0* (delta)    * ( ( *A_n_pml )( i+2, j )-( *A_n_pml )( i-2, j ) )/(4.*dx)
                                                        - k0*k0*( *A_n_pml )( i, j ) ;
                    // d2A/dy^2 = d2A/dy^2
