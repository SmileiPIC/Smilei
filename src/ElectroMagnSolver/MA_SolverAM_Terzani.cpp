...
#include "dcomplex.h"
#include "Patch.h"
MA_SolverAM_Terzani::MA_SolverAM_Terzani( Params &params )
    : SolverAM( params )
{
    delta = ( pow(dt_ov_dl,2)-1.) / 24. ;
}

MA_SolverAM_Terzani::~MA_SolverAM_Terzani()
{
}

void MA_SolverAM_Terzani::operator()( ElectroMagn *fields )
{
    ...
    for( unsigned int imode=0 ; imode<Nmode ; imode++ ) {
        ...
        for( unsigned int i=1 ; i<nl_p-1 ; i++ ) {
            for( unsigned int j=isYmin*3 ; j<nr_d ; j++ ) {
                ( *Er )( i, j ) += -dt*( *Jr )( i, j )
                                   -  (1-3.*delta) *  dt_ov_dl * ( ( *Bt )( i+1, j ) - ( *Bt )( i  , j ) )
                                   -  delta        *  dt_ov_dl * ( ( *Bt )( i+2, j ) - ( *Bt )( i-1, j ) )
        ...
        for( unsigned int i=1 ;  i<nl_p-1 ; i++ ) {
            for( unsigned int j=isYmin*3 ; j<nr_p ; j++ ) {
                ( *Et )( i, j ) += -dt*( *Jt )( i, j )
                                   +  (1-3.*delta) *  dt_ov_dl * ( ( *Br )( i+1, j ) - ( *Br )( i  , j ) )
                                   +   delta       *  dt_ov_dl * ( ( *Br )( i+2, j ) - ( *Br )( i-1, j ) )
