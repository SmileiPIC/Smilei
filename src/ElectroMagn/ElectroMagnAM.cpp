...
void ElectroMagnAM::applyExternalTimeFields( Patch *patch, double time_prim )
{
#ifdef _TODO_AM
#endif
    int Nmodes = El_.size();
    ...
    for (int imode=0;imode<Nmodes;imode++){
        for( vector<ExtField>::iterator extfield=extFields.begin(); extfield!=extFields.end(); extfield++ ) {
            ...
            if( field ){ 
                applyExternalTimeField( field, extfield->profile, patch, time_prim );
            };
        }
        Bl_m[imode]->copyFrom( Bl_[imode] );
        Br_m[imode]->copyFrom( Br_[imode] );
...
void ElectroMagnAM::applyExternalTimeField( Field *my_field,  Profile *profile, Patch *patch, double time_prim )
{
