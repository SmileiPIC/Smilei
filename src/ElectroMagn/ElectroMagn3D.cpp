...
void ElectroMagn3D::initElectroMagn3DQuantities( Params &params, Patch *patch )
{
    ...
    nx_d = size_[0]+2+2*oversize[0]-(params.is_pxr);
    // number of nodes of the primal and dual grid in the y-direction
    ny_p = size_[1]+1+2*oversize[1];
    ny_d = size_[1]+2+2*oversize[1]-(params.is_pxr);
    // number of nodes of the primal and dual grid in the z-direction
    nz_p = size_[2]+1+2*oversize[2];
    nz_d = size_[2]+2+2*oversize[2]-(params.is_pxr);
