...
public:
    ...
    int (*bc_west)  ( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    //! East particles boundary conditions pointers
    int (*bc_east)  ( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    //! South particles boundary conditions pointers
    int (*bc_south) ( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    //! North particles boundary conditions pointers
    int (*bc_north) ( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    //! Bottom particles boundary conditions pointers
    int (*bc_bottom)( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    //! Up particles boundary conditions pointers
    int (*bc_up)    ( Particles &particles, int ipart, int direction, double limit_pos, double &nrj_iPart );
    ...
    inline int apply( Particles &particles, int ipart, double &nrj_iPart ) {//, bool &contribute ) {

        int keep_part = 1;
        if ( particles.position(0, ipart) <  x_min ) {
            if (bc_west==NULL) keep_part = 0;
            else {
                keep_part = (*bc_west)( particles, ipart, 0, 2.*x_min, nrj_iPart );
            }
        }
        else if ( particles.position(0, ipart) >= x_max ) {
            if (bc_east==NULL) keep_part = 0;
            else {
                keep_part = (*bc_east)( particles, ipart, 0, 2.*x_max, nrj_iPart );
            }
        }
        if (nDim_particle >= 2) {
		...
		if (bc_south==NULL) keep_part = 0;
                    ...
                    keep_part *= (*bc_south)( particles, ipart, 1, 2.*y_min, nrj_iPart );
                }
            }
            else if ( particles.position(1, ipart) >= y_max ) {
		if (bc_north==NULL) keep_part = 0;
                else {
                    keep_part *= (*bc_north)( particles, ipart, 1, 2.*y_max, nrj_iPart );
                }
            }
            ...
            if (nDim_particle == 3) {
                ...
                if ( particles.position(2, ipart) <  z_min ) {
                    if (bc_bottom==NULL) keep_part = 0;
                        ...
                        keep_part *= (*bc_bottom)( particles, ipart, 2, 2.*z_min, nrj_iPart );
                    }
                }
                else if ( particles.position(2, ipart) >= z_max ) {
                    if (bc_up==NULL) keep_part = 0;
                    else {
			keep_part *= (*bc_up)( particles, ipart, 2, 2.*z_max, nrj_iPart );
                    }
                }
