...
{
    DEBUG( "Creating the FullPPT Tunnel Ionizaton class" );
    ...
    for( unsigned int i=0; i<atomic_number_; i++ ) {
        DEBUG( "ioniz: i " << i << " potential: " << Potential[i] << " Az.q.num: " << Azimuthal_quantum_number[i] << " M.q.num: " << Magnetic_quantum_number[i] );
    }
...
void IonizationTunnelFullPPT::operator()( Particles *particles, unsigned int ipart_min, unsigned int ipart_max, vector<double> *Epart, Patch *patch, Projector *Proj, int ipart_ref )
{
