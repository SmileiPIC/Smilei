...
IonizationTunnelTL::IonizationTunnelTL( Params &params, Species *species ) : Ionization( params, species )
{
    ...
    ionization_tl_parameter_ = species->ionization_tl_parameter_; // species->ionization_tl_parameter_ is double
...
void IonizationTunnelTL::operator()( Particles *particles, unsigned int ipart_min, unsigned int ipart_max, vector<double> *Epart, Patch *patch, Projector *Proj, int ipart_ref )
{

    unsigned int Z, Zp1, newZ, k_times;
    double TotalIonizPot, E, invE, factorJion, delta, ran_p, Mult, D_sum, P_sum, Pint_tunnel;
    vector<double> IonizRate_tunnel_TL( atomic_number_ ), Dnom_tunnel( atomic_number_ );
    LocalFields Jion;
    ...
    for( unsigned int ipart=ipart_min ; ipart<ipart_max; ipart++ ) {
        ...
        IonizRate_tunnel_TL[Z] = beta_tunnel[Z] * exp( -delta*one_third + alpha_tunnel[Z]*log( delta ) - E*lambda_tunnel[Z] );
        ...
        if( Zp1 == atomic_number_ ) {
            ...
            // -----------------------------------------------------
            if( ran_p < 1.0 -exp( -IonizRate_tunnel_TL[Z]*dt ) ) {
            ...
            Mult = 1.0;
            Dnom_tunnel[0]=1.0;
            Pint_tunnel = exp( -IonizRate_tunnel_TL[Z]*dt ); // cummulative prob.
            ...
            while( ( Pint_tunnel < ran_p ) and ( k_times < atomic_number_-Zp1 ) ) {
                ...
                IonizRate_tunnel_TL[newZ] = beta_tunnel[newZ]
                                            * exp( - delta*one_third + alpha_tunnel[newZ]*log( delta ) - E*lambda_tunnel[newZ] );
                D_sum = 0.0;
                P_sum = 0.0;
                Mult  *= IonizRate_tunnel_TL[Z+k_times];
                for( unsigned int i=0; i<k_times+1; i++ ) {
                    Dnom_tunnel[i]=Dnom_tunnel[i]/( IonizRate_tunnel_TL[newZ]-IonizRate_tunnel_TL[Z+i] );
                    D_sum += Dnom_tunnel[i];
                    P_sum += exp( -IonizRate_tunnel_TL[Z+i]*dt )*Dnom_tunnel[i];
                }
                Dnom_tunnel[k_times+1]  = -D_sum; //bug fix
                P_sum                   = P_sum + Dnom_tunnel[k_times+1]*exp( -IonizRate_tunnel_TL[newZ]*dt );
