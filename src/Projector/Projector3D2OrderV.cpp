...
// ---------------------------------------------------------------------------------------------------------------------
void Projector3D2OrderV::basic( double *rhoj, Particles &particles, unsigned int ipart, unsigned int type, int bin_shift )
{
...
void Projector3D2OrderV::susceptibility( ElectroMagn *EMfields, Particles &particles, double species_mass, SmileiMPI *smpi, int istart, int iend,  int ithread, int scell, int ipart_ref )
{
    ...
    for( int ivect=0 ; ivect < cell_nparts; ivect += vecSize ) {
        ...
        for( int ipart=0 ; ipart<np_computed; ipart++ ) {
            ...
            momentum[0] = momentum_x[istart0+ipart];
            momentum[1] = momentum_y[istart0+ipart];
            ...
            // locate the particle on the primal grid at current time-step & calculate coeff. S1
            xpn = position_x[istart0+ipart] * dx_inv_;
            int ip = round( xpn );
            delta  = xpn - ( double )ip;
            delta2 = delta*delta;
            ...
            ypn = position_y[istart0+ipart] * dy_inv_;
            int jp = round( ypn );
            delta  = ypn - ( double )jp;
            delta2 = delta*delta;
            ...
            zpn = position_z[istart0+ipart] * dz_inv_;
            int kp = round( zpn );
            delta  = zpn - ( double )kp;
            delta2 = delta*delta;
