...
// ---------------------------------------------------------------------------------------------------------------------
void Projector3D4OrderV::ionizationCurrentsForTasks( double *b_Jx, double *b_Jy, double *b_Jz, Particles &particles, int ipart, LocalFields Jion, int bin_shift )
{
...
//! For Order 4, it is more efficient for vectorization to recompute
//! the coefficients before projecting each current component.
//! This is due to the fact that the buffers are too big
//! and generate memory issues at cache levels.
// ---------------------------------------------------------------------------------------------------------------------
void Projector3D4OrderV::currents( double * __restrict__ Jx,
                                   double     * __restrict__ Jy,
                                   double     * __restrict__ Jz,
                                   Particles  &particles,
...
// ---------------------------------------------------------------------------------------------------------------------
void Projector3D4OrderV::currentsAndDensityForTasks( double *Jx, double *Jy, double *Jz, double *rho, int bin_shift, Particles &particles, unsigned int istart, unsigned int iend, std::vector<double> *invgf, int *iold, double *deltaold, int ipart_ref )
{
    // -------------------------------------
    // Variable declaration & initialization
    // -------------------------------------
...
#pragma unroll(8)
    ...
    iloc = iloc0+(ipom2-bin_shift)*nprimz;
    for( unsigned int i=0 ; i<7 ; i++ ) {
        for( unsigned int j=1 ; j<7 ; j++ ) {
            #pragma omp simd
            for( unsigned int k=0 ; k<7 ; k++ ) {
...
#pragma unroll(8)
    ...
    iloc = iloc0  + jpom2 +(ipom2-bin_shift)*nprimy;
    for( unsigned int i=0 ; i<7 ; i++ ) {
        for( unsigned int j=0 ; j<7 ; j++ ) {
            #pragma omp simd
            for( unsigned int k=1 ; k<7 ; k++ ) {
...
// ---------------------------------------------------------------------------------------------------------------------
void Projector3D4OrderV::currentsForTasks( double *Jx, double *Jy, double *Jz, int bin_shift, Particles &particles, unsigned int istart, unsigned int iend, std::vector<double> *invgf, int *iold, double *deltaold, int ipart_ref )
{
    // -------------------------------------
    // Variable declaration & initialization
    // -------------------------------------
