...
namespace naive {
    ...
    #pragma acc parallel present( iold_ [0:3 * nparts],     \
                                  deltaold [0:3 * nparts], \
                                  Sx0 [0:kTmpArraySize],   \
                                  Sy0 [0:kTmpArraySize],   \
                                  Sz0 [0:kTmpArraySize],   \
    ...
    #pragma acc loop gang worker vector
        ...
        for( int ipart=istart_pack ; ipart<iend_pack; ipart++ ) {
            ...
            const int ipo = iold_[0*packsize+ipart];
            ...
            const int jpo = iold_[1*packsize+ipart];
            ...
            const int kpo = iold_[2*packsize+ipart];
            ...
            iold_[ipart+0*packsize] -= 2;   //This minus 2 come from the order 2 scheme, based on a 5 points stencil from -2 to +2.
            // i/j/kpo stored with - i/j/k_domain_begin_ in Interpolator
            iold_[ipart+1*packsize] -= 2;
            iold_[ipart+2*packsize] -= 2;
        }
    ...
    #pragma acc loop gang worker
        ...
        for( int ipart=istart_pack ; ipart<iend_pack; ipart++ ) {
            ...
            const int linindex0 = iold_[ipart+0*packsize]*yz_size0+iold_[ipart+1*packsize]*z_size0+iold_[ipart+2*packsize];
    ...
    #pragma acc loop gang worker
        ...
        for( int ipart=istart_pack ; ipart<iend_pack; ipart++ ) {
            ...
            const int linindex1 = iold_[ipart+0*packsize]*yz_size1+iold_[ipart+1*packsize]*z_size1+iold_[ipart+2*packsize];
#ifdef SMILEI_OPENACC_MODE
            #pragma acc loop vector
#endif
            for( int k=0 ; k<5 ; k++ ) {
    ...
    #pragma acc loop gang worker
        ...
        for( int ipart=istart_pack ; ipart<iend_pack; ipart++ ) {
            ...
            const int linindex2 = iold_[ipart+0*packsize]*yz_size2+iold_[ipart+1*packsize]*z_size2+iold_[ipart+2*packsize];
#ifdef SMILEI_OPENACC_MODE
            #pragma acc loop vector
#endif
            for( int k=1 ; k<5 ; k++ ) {
...
namespace hip {
    ...
    namespace kernel {
        ...
        DepositCurrentDensity_3D_Order2( double *__restrict__ device_Jx,
                                         ...
                                         int *__restrict__ device_ iold_,
