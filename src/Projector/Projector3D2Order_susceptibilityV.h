#ifndef PROJECTOR3D2ORDERSUSCEPTIBILITYV_H
#define PROJECTOR3D2ORDERSUSCEPTIBILITYV_H

#include "Projector3D.h"


class Projector3D2Order_susceptibilityV : public Projector3D {
public:
    Projector3D2Order_susceptibilityV(Params&, Patch* patch);
    ~Projector3D2Order_susceptibilityV();
    ...
    void project_susceptibility(double* Chi_envelope, Particles &particles, int istart, int iend, unsigned int bin, std::vector<unsigned int> &b_dim, SmileiMPI* smpi, int ithread, double species_mass, int* iold, int ipart_ref);
