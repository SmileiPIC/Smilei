...
void PusherPonderomotiveBoris::operator()( Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, int ipart_buffer_offset )
{
    ...
    //double *inv_gamma_ponderomotive = &( ( *dynamics_inv_gamma_ponderomotive )[0*nparts] );
        ...
        int np = iend-istart;
        #pragma acc parallel \
        present(Ex[istart:np],Ey[istart:np],Ez[istart:np],Bx[istart:np],By[istart:np],Bz[istart:np],dynamics_inv_gamma_ponderomotive[0:nparts],GradPhix[istart:np],GradPhiy[istart:np],GradPhiz[istart:np]) \
        deviceptr(momentum_x,momentum_y,momentum_z,charge)
