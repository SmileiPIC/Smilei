...
//    DEBUG("--------------------------------------------------------------");
    ...
    probeFileGlobal_id = H5Fcreate( file_name.str().c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
...
void DiagnosticProbe0D::run2(int timestep, ElectroMagn* EMfields, Interpolator* interp){
  ...
  for (unsigned int count=0; count <probeParticles.size(); count++) {
	...
	H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, partMemSpace, file_space, H5P_DEFAULT,data);
